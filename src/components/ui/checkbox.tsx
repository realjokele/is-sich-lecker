...
const boxStyles = tv({
  base: 'border-input text-bg flex size-4 shrink-0 items-center justify-center rounded border transition *:data-[slot=icon]:size-3',
  variants: {
    isSelected: {
      false: 'bg-muted',
      true: [
    ...
    isFocusVisible: {
      true: [
        'border-primary ring-primary/70 ring-2 ring-offset-1',
      ...
      {({ isSelected, isIndeterminate, ...renderProps }) => (
        <div className={cn('flex gap-x-2', props.description ? 'items-start' : 'items-center')}>
          ...
          <div className="flex flex-col gap-1">
            <>
              {props.label ? (
                <Label className={cn(props.description && 'text-sm/4 font-normal')}>{props.label}</Label>
              ) : (
                (props.children as React.ReactNode)
              )}
